#!/usr/bin/env python
"""Train IQL on LeRobot-compatible navigation dataset.
...
def main():
    parser = argparse.ArgumentParser(description="Train IQL")
    parser.add_argument("--data-dir", type=str, required=True)
    parser.add_argument("--config", type=str, default="configs/new/iql_config.yaml")
    ...
    # Build replay buffer
    # Convert LeRobot frames to unified format
    T = len(frames)
    obs = {
        "rgb": None,
        "scan64": None,
    ...
    for t in range(T):
        ...
        if "observation.images.rgb" in f:
            rgb = f["observation.images.rgb"]  # (C, H, W) float [0,1]
            rgb_np = (rgb.permute(1, 2, 0).numpy() * 255).astype(np.uint8)
            if obs["rgb"] is None:
                obs["rgb"] = np.zeros((T, *rgb_np.shape), dtype=np.uint8)
            obs["rgb"][t] = rgb_np

        if "observation.scan64" in f:
            if obs["scan64"] is None:
                obs["scan64"] = np.zeros((T, f["observation.scan64"].shape[0]), dtype=np.float32)
            obs["scan64"][t] = f["observation.scan64"].numpy()

        if "observation.state" in f:
            if obs["state"] is None:
                obs["state"] = np.zeros((T, 3), dtype=np.float32)
            obs["state"][t] = f["observation.state"].numpy()

        if "observation.goal" in f:
            if obs["goal"] is None:
                obs["goal"] = np.zeros((T, 3), dtype=np.float32)
            obs["goal"][t] = f["observation.goal"].numpy()
    ...
    # Create IQL trainer
    iql = IQL(model, config, device=args.device)
    ...
    for epoch in range(num_epochs):
        epoch_losses = {"value_loss": 0.0, "critic_loss": 0.0, "actor_loss": 0.0}
        ...
        print(f"Epoch {epoch + 1}/{num_epochs}: "
              f"V={epoch_losses['value_loss']:.4f} "
        ...
        if total_loss < best_loss:
            ...
            torch.save(iql.state_dict(), os.path.join(args.output_dir, "best.pt"))
    ...
    # Final save
    torch.save(iql.state_dict(), os.path.join(args.output_dir, "final.pt"))
